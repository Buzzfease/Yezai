...
public class PicSwipeCallBack extends ItemTouchHelper.Callback {

    private int dragFlags;
    private int swipeFlags;
    private PreviewSelectAdapter adapter;
    private List<String> images;//图片地址
    private boolean up;//手指抬起标记位

    public PicSwipeCallBack(PreviewSelectAdapter adapter, List<String> images) {
    ...
    public void onChildDraw(Canvas c, RecyclerView recyclerView, RecyclerView.ViewHolder viewHolder, float dX, float dY, int actionState, boolean isCurrentlyActive) {
        ...
        if (dY >= (recyclerView.getHeight()
                ...
                - DisplayUtil.INSTANCE.dip2px(MyApplication.Companion.instance(),  44f))) {
            //拖到删除处
